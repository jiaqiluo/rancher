...
func ToCapiObjRef(ref *corev1.ObjectReference) (*capi.ContractVersionedObjectReference, error) {
	if ref == nil {
		return nil, nil
	}
...
func durationToSeconds(d *metav1.Duration, cluster *rancherv1.Cluster) *int32 {
	...
	if s > maxInt32 {
		logrus.Warnf("cluster %s/%s: drainBeforeDeleteTimeout %s is greater than max supported value, "+
			"capping to max value %d", cluster.Namespace, cluster.Name, d.Duration.String(), int32(maxInt32))
		s = maxInt32
	}
	seconds := int32(s)
	return &seconds
