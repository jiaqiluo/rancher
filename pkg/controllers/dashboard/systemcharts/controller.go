...
func (h *handler) onRepo(key string, repo *catalog.ClusterRepo) (*catalog.ClusterRepo, error) {
	...
	for _, chartDef := range h.getChartsToInstall() {
		if chartDef.Uninstall {
			logrus.Infof("===== attempting to uninstall the chart %s", chartDef.ChartName) // TODO: remove the logging
		...
		logrus.Infof("===== attempting to install the chart %s", chartDef.ChartName) // TODO: remove the logging
...
func (h *handler) onCluster(_ string, obj *v3.Cluster) (*v3.Cluster, error) {
	...
	logrus.Debugf("[systemcharts] local cluster has been updated, enqueue %s", repoName)
	h.clusterRepo.EnqueueAfter(repoName, 2*time.Second)
	return obj, nil
}
